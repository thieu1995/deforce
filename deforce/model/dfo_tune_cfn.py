#!/usr/bin/env python
# Created by "Thieu" at 21:21, 06/04/2024 ----------%                                                                               
...
class DfoTuneCfn:
    ...
    def __init__(self, problem_type="regression", bounds=None, cv=5, scoring="MSE",
        ...
        if problem_type == "regression":
            self.network_class = CfnRegressor
            self.scoring = validators.check_str("scoring", scoring, self.SUPPORTED_REG_METRICS)
            self.minmax = self.SUPPORTED_REG_METRICS[self.scoring]
            self.metric_class = RegressionMetric
        else:
            self.network_class = CfnClassifier
            self.scoring = validators.check_str("scoring", scoring, self.SUPPORTED_CLS_METRICS)
